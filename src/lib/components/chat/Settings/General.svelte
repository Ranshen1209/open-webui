			...
			{#if $i18n.language === 'en-US' && !($config?.license_metadata ?? false)}
				...
				>
					...
					<a
						...
						target="_blank"
					>
						Help us translate Sakrylle Web!
					</a>
				</div>
			{/if}
				...
				<Textarea
					...
					className={'w-full text-sm outline-hidden resize-vertical' +
						($settings.highContrastMode
							? ' p-2.5 border-2 border-gray-300 dark:border-gray-700 rounded-lg bg-transparent text-gray-900 dark:text-gray-100 focus:ring-1 focus:ring-primary-500 focus:border-primary-500 overflow-y-hidden'
							: '  dark:text-gray-300 ')}
					rows="4"
