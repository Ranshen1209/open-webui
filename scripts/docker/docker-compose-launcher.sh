...
if [[ "$drop_project" == true ]]; then
  (cd "$REPO_ROOT" && "${DOCKER_COMPOSE[@]}" -f docker-compose.yaml down --remove-orphans)
...
if [[ "$enable_gpu" == true ]]; then
  ...
  compose_files+=("-f" "deploy/compose/docker-compose.gpu.yaml")
fi

if [[ "$enable_api" == true ]]; then
  export OLLAMA_WEBAPI_PORT="$api_port"
  compose_files+=("-f" "deploy/compose/docker-compose.api.yaml")
fi

if [[ -n "$data_dir" ]]; then
  export OLLAMA_DATA_DIR="$data_dir"
  compose_files+=("-f" "deploy/compose/docker-compose.data.yaml")
fi

if [[ "$enable_playwright" == true ]]; then
  compose_files+=("-f" "deploy/compose/docker-compose.playwright.yaml")
fi
...
if (cd "$REPO_ROOT" && "${DOCKER_COMPOSE[@]}" "${compose_files[@]}" "${up_args[@]}"); then
  echo
